...
Get4KPteFromAddress (EFI_PHYSICAL_ADDRESS Address) {
    ...
    if (Pte2M->Bits.MustBe1) {    // Page must have been converted to 4K
        return NULL;
    }
...
InitializeSpecialPool (
    ...
    // During SMM module loading, UEFI switches to the page table for SMM.  This confuses special pool somewhat,
    // so only update the "master" page tables when EnablePage/DisablePage occurs.
    gMasterCr3 = AsmReadCr3();
