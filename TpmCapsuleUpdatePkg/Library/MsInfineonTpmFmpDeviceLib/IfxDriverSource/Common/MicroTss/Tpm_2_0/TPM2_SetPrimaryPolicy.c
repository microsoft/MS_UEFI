/**
 *	@brief		Implements the TPM2_SetPrimaryPolicy method
 *	@file		TPM2_SetPrimaryPolicy.c
...
)
{
	...
	do
	{
		...
		TPM_CC commandCode = TPM_CC_SetPrimaryPolicy;
		// Response parameters
		...
		unReturnValue = TSS_TPM2B_DIGEST_Marshal(&authPolicy, &pbBuffer, &nSizeRemaining);
		if (RC_SUCCESS != unReturnValue)
			break;
		unReturnValue = TSS_TPMI_ALG_HASH_Marshal(&hashAlg, &pbBuffer, &nSizeRemaining);
		if (RC_SUCCESS != unReturnValue)
			break;
