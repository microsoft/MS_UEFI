...
)
{
	...
	for (Index = 0; Index < fm_cap_header->EmbeddedDriverCount; Index++)
	{
		...
		fprintf(stdout, "Embedded Driver %d at Offset: 0x%llx\n", Index, Offset);
	...
	for (Index = 0; Index < fm_cap_header->PayloadItemCount; Index++)
	{
		...
		fprintf(stdout, "Payload Item %d at Offset: 0x%llx\n", Index, Offset);
...
--*/
{
  ...
  for (Index = 0; Index < DriverFreeIndex; Index++)
  {
	  ...
	  tread2 = DriverFileList[Index].DataSize;

	  //read it
	  readsize2 = 0;
	  while (readsize2 < tread2)
	  {
		  readsize2 += fread((DriverFileList[Index].FileData + readsize2), sizeof(UINT8), (tread2 - readsize2), FpFile);
	  }
	  fclose(FpFile);
